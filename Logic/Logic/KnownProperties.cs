...
public static class KnownProperties
{
	...
	static KnownProperties()
	{
		Dictionary<string, string> dictionary = typeof(KnownProperties).
												...
												ToDictionary(
															...
															fieldInfo => ((string)fieldInfo.GetValue(null) ).ToPropertyName( fieldInfo.GetCustomAttribute<PropertyAttribute>().Namespace));

		PropertiesList = new ReadOnlyDictionary<string, string>(dictionary);
	}

	[Property( EntityScope.DirectoryService, KnownNamespaces.DirectoryServicesNamespace, true)]
	public const string ApiEndpoints = nameof(ApiEndpoints);


	public static ReadOnlyCollection<(string HostName, int Port)> ParseApiEndpoints ( string value )
	{
	...
	public static string GetPropertyName ( this string propertyName ) => PropertiesList[propertyName] ;
