...
public class ServerLoginProvider : ILoginProvider
{
	...
	public static void Prepare ( )
	{
		...
		{
			LoginServices = new ReadOnlyDictionary <Guid , LoginServiceBase> (
			AppDomainExtensions .
				FindType ( type => type . IsAssignableTo ( typeof ( LoginServiceBase ) ) ) .
				Distinct ( ) .
				Select ( Activator . CreateInstance ) .
				Cast <LoginServiceBase> ( ) .
				ToDictionary ( loginService => loginService . Type ) ) ;
		}
	}
