...
namespace DreamRecorder.Directory.Services.Logic
{
	...
	[PublicAPI]
	public abstract class DirectoryServiceBase : ServiceBase, IDirectoryService, IDirectoryServiceInternal
	{
		...
		public abstract IAccessTokenProvider AccessTokenProvider { get; }

		public abstract IEntityTokenProvider EntityTokenProvider { get; }

		public EntityToken ServiceToken => EntityTokenProvider.GetToken();

		public abstract ILoginServiceProvider LoginServiceProvider { get; }

		public abstract IDirectoryServiceProvider DirectoryServiceProvider { get; }

		public abstract EntityToken AnonymousToken { get; }

		public abstract ITokenStorage<EntityToken> IssuedEntityTokens { get; }

		public abstract ITokenStorage<AccessToken> IssuedAccessTokens { get; }

		public abstract IDirectoryDatabase DirectoryDatabase { get; }

		public abstract DirectoryService ServiceEntity { get; }

		public abstract ITokenPolicy TokenPolicy { get; }

		public IDirectoryService DirectoryService => this;
		...
		public void DisposeToken([NotNull] EntityToken token)
		{
			...
			{
				DirectoryService issuer = DirectoryDatabase.FindEntity(token.Issuer) as DirectoryService;
		...
		public void DisposeToken(LoginToken token)
		{
			...
			LoginService issuer = DirectoryDatabase.FindEntity(token.Issuer) as LoginService;
		...
		public void TransferProperty(EntityToken token, Guid target, string name, Guid newOwner)
		{
			...
			if (requester != null)
			{
				...
				if (propertyOwner != null)
				{
					...
					if (property != null)
					{
						if (DirectoryDatabase.FindEntity(property.Owner).Contain(requester))
						{
		...
		public PermissionGroup UpdatePermissionGroup(EntityToken token, PermissionGroup target)
		{
			...
			if (requester != null)
			{
				...
				if (targetPermissionGroup != null)
				{
					if (DirectoryDatabase.FindEntity(targetPermissionGroup.Owner).Contain(requester))
					{
		...
		public void CheckToken(EntityToken token, EntityToken tokenToCheck)
		{
			...
			DirectoryService directoryService = DirectoryDatabase.FindEntity(token.Issuer) as DirectoryService;
		...
		public Guid CreateUser(EntityToken token)
		{
			...
			LoginService requester = DirectoryDatabase.FindEntity(token.Owner) as LoginService;
